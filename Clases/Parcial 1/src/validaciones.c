...
static int getEmail (char* pCadena, int len);
...
static int esEmail( char * cadena, int len);
...
int compararMismoCaracteres (char* cadenaUno, char* cadenaDos, int len)
{
	...
	if (cadenaUno != NULL && cadenaDos != NULL)
	{
		comparacion = strcmp (cadenaUno,cadenaDos);
...
int utn_getEmail(char* charIngresado, char* mensaje, char* mensajeError, int min, int max, int reintentos)
{
	...
	if (mensaje != NULL && mensajeError != NULL && min < max && reintentos > 0)
	{
		while (reintentos > 0)
		{
			reintentos--;
			printf (mensaje);
			if (getEmail (buffer, sizeof(buffer)) == 1 && strlen(buffer)>= min && strlen(buffer) < max)
			{
				strncpy(charIngresado , buffer, max );
				rtn = 0;
				break;
...
static int getEmail (char* pCadena, int len)
{
	...
	if ( pCadena != NULL &&
		 myGets(buffer, sizeof (buffer)) == 1 &&
		 esEmail(buffer, sizeof(buffer)) == 1 &&
		 strnlen(buffer,sizeof(buffer)) < len)
	{
		strncpy (pCadena, buffer, len);
		rtn = 1;
...
static int esEmail ( char * cadena, int len)
{
	...
	if (cadena != NULL && len > 0)
	{
		while (cadena[i] != '\0' && i < len)
		{
			if (flagArroba > 0 && flagPunto == 0) contador++;

			if(cadena[i] == '@') flagArroba ++;

			if(cadena[i] == '.' && flagPunto == 0 && flagArroba == 1) flagPunto++;
		...
		if (flagArroba != 1 || flagPunto == 0 || contador < 2 )
		{
			rtn = -1;
		}
	}
...
void menuUsuario (void)
{
	...
	puts ("5) MODIFICAR DATOS USUARIO\n");
	puts ("0) SALIR");
}
